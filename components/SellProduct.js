...
import { cartContext, languageContext, userContext } from "@/pages/_app";
...
const SellProduct = ({ loader, toaster }) => {
  ...
  const handleAddToCart = (item) => {
    // Check if user is logged in and document verified before allowing add to cart
    if (!user || !user._id || !user.documentVerified) {
      toaster({
        type: "error",
            ...
            {saleData.map((item, i) => {
              ...
              return (
                ...
                >
                  ...
                  {/* Price Section */}
                  {user && user._id && user.documentVerified ? (
                    ...
                    <div className="flex flex-col items-center mb-3">
                      <p className="text-orange-500 font-medium md:text-xs text-[10px]">
                        Verification pending - Complete to view prices
                      </p>
                    </div>
                  ) : (
                    <div className="flex flex-col items-center mb-3">
                      <p className="text-custom-green font-medium md:text-xs text-[10px]">
